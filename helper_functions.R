...
get_ghcnd <- function(station_id = "JA000047759",
  ...
  weather <- ghcnd_search(
    stationid = station_id, var = c("tmax"),
    date_min = date_range[1], date_max = date_range[2]
  )[[1]]

  for (info in c("tmin", "prcp")) {
    info_tmp <- ghcnd_search(
      stationid = station_id, var = c(info),
      date_min = date_range[1], date_max = date_range[2]
    )[[1]]
...
get_afdd <- function(dat) {
  ...
  freeze_days_n <- freeze_days %>%
    group_by(year) %>%
    summarise(afdd = n())

  # First and last freeze
  first_freeze <- freeze_days %>%
    group_by(year) %>%
    summarise(first_freeze_date = min(date))
  last_freeze <- freeze_days %>%
    group_by(year) %>%
    summarise(last_freeze_date = max(date))
...
get_agdd <- function(dat) {
  growing_days <- dat %>%
    mutate(yday = yday(date)) %>%
    group_by(year) %>%
    mutate(tavg_cal = (tmax_imp + tmin_imp) / 2) %>%
    filter(tavg_cal > 0)

  # Number of accumulative growing days
  grow_days_n <- growing_days %>%
    group_by(year) %>%
    summarise(agdd = n())

  # First and last freeze
  first_grow <- growing_days %>%
    group_by(year) %>%
    summarise(first_grow_date = min(date))
  last_grow <- growing_days %>%
    group_by(year) %>%
    summarise(last_grow_date = max(date))
...
Three_Cities_kernel_fun <- function(loc, final.dat, bw) {
  ...
  estimated.intercept <- Reduce("rbind", lapply(beta_hat, FUN = function(x) {
    return(x["Intercept", ])
  }))
  estimated.afdd <- Reduce("rbind", lapply(beta_hat, FUN = function(x) {
    return(x["afdd", ])
  }))
  estimated.tmin_Spring <- Reduce("rbind", lapply(beta_hat, FUN = function(x) {
    return(x["tmin_Spring", ])
  }))
  estimated.tmax_Spring <- Reduce("rbind", lapply(beta_hat, FUN = function(x) {
    return(x["tmax_Spring", ])
  }))
