...
library(tidyverse)
library(tinytable) # Check if these need to be loaded
...
Currently **only three observations** for 2022, 2023, and 2024 are available.

But casual observations have been recorded in the form of **photos posted** to the [VCBF Neighbourhood Blog for Kerrisdale](https://forums.botanicalgarden.ubc.ca/threads/kerrisdale.36008/).
...
Then we add these observations to the data set
...
The data in file `washingtondc.csv` has been obtained from the EPA's Climate Change Indicators in the United States <https://www.epa.gov/climate-indicators/cherry-blossoms>.
...
## Visualizing the five main time series
