...
Note that the `{rnoaa}` package only provides data until about 2022-09-15 with the new NOAA API v2. Since it is not available on Cran anymore, use `devtools::install_github("ropensci/rnoaa")` to install the package from GitHub.

The `{rnpn}` package is available at <https://CRAN.R-project.org/package=rnpn>.

Check out the `{GSOD}` package <https://docs.ropensci.org/GSODR/>.
...
Quetelet reported on the law of the flowering plants in *On the climate of Belgium* (1857). Sources: [Wikimedia Commons](https://en.wikipedia.org/wiki/Adolphe_Quetelet#/media/File:Adolphe_Qu%C3%A9telet_by_Joseph-Arnold_Demannez.jpg), [Gallica](https://gallica.bnf.fr/ark:/12148/bpt6k95028d).
:::
...
Bloom dates at Brussels Observatory observed by Quetelet between 1839 and 1852. Source: [Gallica](https://gallica.bnf.fr/ark:/12148/bpt6k95028d).
:::


## Reproducing Quetelet's law of the flowering plants

To reproduce Quetelet's law, we combine the data in @fig-02 with additional observations from his *Letters*. We focus on Quetelet's primary example, the bloom date of the common lilac, *Syringa vulgaris*, row 18 of @fig-02. We do this because Quetelet carefully describes his methodology for measuring the bloom date of lilacs. For example, Quetelet considers a lilac to have bloomed when "the first corolla opens and shows the stamina." That event is closest to what the USA Phenology Network describes as "open flowers", depicted in the center image of @fig-03 below. This detail will become relevant when we attempt to replicate Quetelet's law in a later section. Note that although we focus on lilacs in this tutorial, the R code is easily edited to predict the day that other plants will bloom.

:::{layout-ncol=3 #fig-03}
...
The bloom date occurs when the first corolla opens and shows the stamina (center image). Source: [USA National Phenology Network](https://www.usanpn.org/nn/TrackaLilac).
:::
...
```{r}
#| label: tbl-01
#| tbl-cap: "Bloom dates of lilacs observed by Quetelet between 1839 and 1852."

lilac <- tibble(
  month = c(
    "May", "April", "April", "April", "April", "April", "May", 
    "April", "May", "April", "May", "April", "May", "May"
    ),
  day   =  c(10, 28, 24, 28, 20, 25, 13, 12, 09, 21, 02, 30, 01, 12),
  year  = 1839:1852,
...
```{r}
#| label: tbl-02
#| tbl-cap: "Temperature observed at Brussels Observatory between 1839 and 1852."
#| message: false

temp <- ghcnd_search(
  stationid = "BE000006447",
  var = c("tmax", "tmin"),
  date_min = "1839-01-01",
  date_max = "1852-12-31"
  ) |> 
  reduce(.f = left_join, join_by(id, date, mflag, qflag, sflag)) |> 
  transmute(
...
```{r}
#| label: tbl-03
#| tbl-cap: "Last frost date and lilac bloom date at Brussels Observatory in 1839."
...
If Quetelet's law of the flowering plants is correct, @tbl-03 has the following interpretation. On March 8, 1839 the lilacs at Brussels Observatory began "collecting" temperature. The lilacs continued to "collect" temperature until May 11, at which point they exceeded their 4264°C² quota and bloomed. We visualize this theory in @fig-04 with the R packages `{ggplot2}`, a member of the set of packages that constitute the "tidyverse" [-@Wickham2019], and `{plotly}`.

```{r}
#| label: fig-04
#| fig-cap: "According to Quetelet's law, the lilacs bloom when exposed to 4264°C² following the last frost."
...
(temp |> 
   ...
   geom_vline(
     xintercept = as.numeric(c(bloom_day, frost_day)),
     linetype = "dotted"
     )) |> 
  ggplotly() |> 
  add_annotations(
    x = as.numeric(c(frost_day, bloom_day)),
    y = c(-4, -4),
    text = c("last\nfrost", "first\nbloom"),
    font = list(size = 14),
    ay = 0,
...
:::{.figure-caption}
According to Quetelet's law, the lilacs bloom when exposed to 4264°C² following the last frost. Author provided, [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/).
:::
...
```{r}
#| label: tbl-04
#| tbl-cap: "Reproduction of Quetelet's analysis."

quetelet <- temp %>%
  group_by(year) %>%
  nest() %>%
  left_join(lilac, by = join_by(year)) %>%
  mutate(
    law = map(
      .x = data, 
      .f = ~ sum(pmax(.$temp, 0, na.rm = TRUE)[(doy_last_frost(tmax = .$tmax) + 1):doy]^2, na.rm = TRUE)
    )
  ) %>%
  unnest(law) %>%
  ungroup()
...
@tbl-05 indicates that predictions made using the law are accurate to within a week on average. For comparison purposes, we also predict the day the lilacs will bloom using the average bloom date between 1839 and 1852. That is, on average the lilac bloomed on April 30 (April 29 on leap years), and we check the accuracy of simply predicting this average date each year. @tbl-06 indicates the average bloom date yields predictions that are less accurate by an average of two days.

```{r}
#| label: tbl-06
#| tbl-cap: "Predictions using the average bloom date are off by a week or more on average."
...
@tbl-07 displays six of the seven columns (and only the first 10 rows of the full table). These columns are defined in the same way as the columns of @tbl-01, except for "site_id", which denotes the site at which the observation was made. @tbl-01 does not have a "site_id" column because all observations were made at the same site, Brussels Observatory.

```{r}
#| label: tbl-07
#| tbl-cap: "Bloom dates of lilacs observed in pacific northwest between 1956 and 2009."
...
The seventh column we review is `temp`. Each row of `temp` is a tibble of temperature records taken at the nearest station in the Global Historical Climatology Network. The first tibble (again, only the first 10 rows) is displayed in @tbl-08 below. The columns are defined in the same way as the columns of @tbl-02, except for `id`, which denotes the location at which the temperature record was made. @tbl-02 does not have an `id` column because all observations were made at the same site, Brussels Observatory.

```{r}
#| label: tbl-08
#| tbl-cap: "Temperature observed at an example pacific northwest site in 1956."
...
usa_npn %>%
  ...
  kable(
    align = "c",
    col.names = c(
      "id", "year", "date", "maximum temperature (°C)",
      "minimum temperature (°C)", "midrange temperature (°C)"
    )
  ) %>%
  kable_styling() %>%
...
@tbl-09 indicates that Quetelet's findings are replicable in the sense that the confidence interval calculated using Quetelet's data (@tbl-04) overlaps with the confidence interval calculated using the USA lilac data (@tbl-09). The standard error in @tbl-09 is smaller than @tbl-04 because the replication uses 54 years of data compared to Quetelet's 14. Note that in the R code above, we subtract 1 from "doy" to correct for differences in how the bloom date is reported. This correction is not particularly important; the confidence intervals still overlap when this correction is removed.
...
```{r}
#| label: tbl-10
#| tbl-cap: "Predictions using Quetelet's law are accurate within about two weeks on average."
...
@tbl-10 indicates that the predictions are accurate to within two weeks on average. Recall that the predictions using Quetelet's own data were accurate to within one week on average (@tbl-05). We speculate that the decrease in accuracy is due in part to the fact that both Quetelet's lilacs and the temperature were observed at the same site, Brussels Observatory. In some cases, the USA lilacs were a few miles from where the temperature was recorded.

Although the accuracy of the predictions made using Quetelet's law is lower when applied to the USA lilac data, @fig-05 indicates that the law produces the correct bloom date on average. The figure plots the predictions made by the law against the actual bloom dates scientists observed. Note that instead of representing prediction-observation pairs as points in a scatter plot, the data are represented using blue contours. We use contours because there are more than 1,500 observations -- too many to study using a scatter plot.

```{r}
#| label: fig-05
#| fig-cap: "Predictions using Quetelet's law are accurate within about two weeks on average."
...
(usa_npn %>%
  ...
  ungroup() %>%
  mutate(
    predicted = as.Date("2020-01-01") + pred,
...
```{r}
#| label: tbl-11
#| tbl-cap: "Predictions using the average bloom date are off by three weeks or more on average."
...
```{r}
#| label: tbl-12
#| tbl-cap: "Temperature forecast for Brussels, retrieved on February 19, 2023."
...
temp_br <- tibble(
  ...
  reduce(bind_rows) %>%
  transmute(
    date = seq(as.Date("2023-01-01"), as.Date("2023-05-31"), 1),
    year = parse_number(format(date, "%Y")),
    tmax,
    tmin,
    temp = (tmax + tmin) / 2
...
We now predict the day the lilacs will bloom. The R code below uses the `doy_prediction` and `doy_last_frost` functions created in earlier sections and displays the prediction in @tbl-13. At the time of our writing, the predicted date is **April 19**. The forecast is easily updated by providing the url to the updated AccuWeather webpage. (You might use the url [https://web.archive.org/save](https://web.archive.org/save) to save a webpage to the Internet Archive to ensure your work is reproducible.)

```{r}
#| label: tbl-13
#| tbl-cap: "Last frost date and lilac bloom date in Brussels in 2023."

bloom_day_br <- temp_br %>%
  summarize(date = doy_prediction(temp, tmax) + as.Date("2023-01-01")) %>%
  pull(date)

frost_day_br <- temp_br %>%
  pull(tmax) %>%
  doy_last_frost() + as.Date("2023-01-01")

tibble(
  `last frost date` = frost_day_br,
...
We visualize the predictions in @fig-06, which has the same interpretation as @fig-04. If the temperature forecast and Quetelet's law are correct, on January 27, 2023 the lilacs in Brussels began "collecting" temperature. The lilacs will continue to "collect" temperature until April 19, at which point they will exceed their 4264°C² quota and bloom.

```{r}
#| label: fig-06
#| fig-cap: "According to Quetelet's law, the lilacs will bloom once exposed to 4264°C² following the last frost."
...
(temp_br %>%
  ...
  geom_vline(
    xintercept = as.numeric(c(frost_day_br, bloom_day_br)),
    linetype = "dotted"
  )) %>%
  ggplotly() %>%
  add_annotations(
    x = as.numeric(c(frost_day_br, bloom_day_br)),
    y = c(14, 14),
    text = c("last\nfrost", "first\nbloom"),
    font = list(size = 14),
    ay = 0,
...
```{r}
#| label: prep-usa-npn-data
#| eval: false

# 1. download lilac data using `{rnpn}`
usa_npn <- npn_download_individual_phenometrics(
  request_source = "Jonathan Auerbach",
  year = 1900:2050,
  species_ids = 36,
...
stations <- ghcnd_stations() %>%
  filter(
    first_year <= min(usa_npn$first_yes_year),
