...
cherry |> 
  ...
  geom_step(linetype = "dotted", colour = "gray50") +
  scale_x_continuous(breaks = c(seq(1880, 2020, by = 20), 2024)) +
  facet_grid(cols = vars(str_to_title(location))) +
  labs(x = "Year", y = "Peak bloom (since Jan 1st)")
```
...
Note: The `{rnoaa}` package is not on CRAN anymore. The following functions allow you to use the NOAA API without the package.

```{r}
nested_to_tibble <- function(x) {
  
  # Determine the variable names in the response
  variable_names <- map(.x = x, .f = names) |>
