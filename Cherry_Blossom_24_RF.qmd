...
city_Colors <- c(
  kyoto = "lightpink1", liestal = "lightskyblue",
  washingtondc = "palegreen3"
)
ggplot(cherry, aes(x = year, y = bloom_doy)) +
  geom_text(
    label = "\u273F", aes(color = location),
  ...
  scale_color_manual(
    values = city_Colors, name = "Location",
...
weather <- rbind(
  weatherWash$tmax, weatherLies$tmax, weatherKyot$tmax,
...
with(temp23, plot(tmax ~ doy.n,
  ylab = "Maximum Temperature (1/10 °Celsius)",
...
for (l in c("Washington", "Liestal", "Kyoto", "Vancouver", "NewYork")) {
  ...
  for (y in 1:(length(unique(weather_Loc$yearWinter)) - 1)) {
    ...
    if (nrow(temp.year) > 200) { # not all years are complete
      ...
      DRD[y] <- as.numeric(optimize(fun,
        interval = c(-91, 200),
        maximum = FALSE
      )[1])
      year[y] <- unique(weather_Loc$yearWinter)[y]
      DRD.t.pred[y] <- as.numeric(optimize(fun,
        interval = c(-91, 200),
        maximum = FALSE
      )[2]) / 10
    }
...
temp_Wash <- data.frame(
  year = year_Washington, DRD = DRD_Washington,
  t.pred = DRD.t.pred_Washington
)
# boxplot.stats(temp_Wash$DRD)$out
temp_Lies <- data.frame(
  year = year_Liestal, DRD = DRD_Liestal,
  t.pred = DRD.t.pred_Liestal
)
# boxplot.stats(temp_Wash$DRD)$out
temp_Kyot <- data.frame(
  year = year_Kyoto, DRD = DRD_Kyoto,
  t.pred = DRD.t.pred_Kyoto
)
# boxplot.stats(temp_Kyot$DRD)$out
temp_Vanc <- data.frame(
  year = year_Vancouver, DRD = DRD_Vancouver,
  t.pred = DRD.t.pred_Vancouver
)
# boxplot.stats(temp_Vanc$DRD)$out
temp_NY <- data.frame(
  year = year_NewYork, DRD = DRD_NewYork,
...
temp$location <- rep(
  c("Washington", "Liestal", "Kyoto", "Vancouver", "NewYork"),
  c(
    nrow(temp_Wash), nrow(temp_Lies), nrow(temp_Kyot),
...
for (y in 1:(length(unique(weather_Wash_jan$yearWinter)) - 1)) {
  ...
  mean.jan_Wash[y] <- mean(weather_Wash_jan[weather_Wash_jan$yearWinter == year, ]$tmax, na.rm = TRUE) / 10
  year_Wash[y] <- year
}
...
for (y in 1:(length(unique(weather_Lies_jan$yearWinter)) - 1)) {
  ...
  mean.jan_Lies[y] <- mean(weather_Lies_jan[weather_Lies_jan$yearWinter == year, ]$tmax, na.rm = TRUE) / 10
  year_Lies[y] <- year
}
...
for (y in 1:(length(unique(weather_kyot_jan$yearWinter)) - 1)) {
  ...
  mean.jan_kyot[y] <- mean(weather_kyot_jan[weather_kyot_jan$yearWinter == year, ]$tmax, na.rm = TRUE) / 10
  year_kyot[y] <- year
}
...
for (y in 1:(length(unique(weather_Vanc_jan$yearWinter)) - 1)) {
  ...
  mean.jan_Vanc[y] <- mean(weather_Vanc_jan[weather_Vanc_jan$yearWinter == year, ]$tmax, na.rm = TRUE) / 10
  year_Vanc[y] <- year
}
...
for (y in 1:(length(unique(weather_NY_jan$yearWinter)) - 1)) {
  ...
  mean.jan_NY[y] <- mean(weather_NY_jan[weather_NY_jan$yearWinter == year, ]$tmax, na.rm = TRUE) / 10
  year_NY[y] <- year
}
...
mw_Wash <- subset(
  aggregate(tmax ~ doy, subset(weather_Wash, year >= 2013), mean),
  doy %in% 61:152
)
...
DRD24_Wash <- as.numeric(optimize(fun24_Wash,
  interval = c(1, 365),
  maximum = FALSE
)[1])
t.DRD24_Wash <- w_Wash[w_Wash$doy.n == round(DRD24_Wash), ]$tmax
tpred24_Wash <- as.numeric(optimize(fun24_Wash,
  interval = c(1, 365),
  maximum = FALSE
)[2])
with(
  subset(w_Wash, doy.n <= 60),
  plot(tmax ~ doy.n,
...
mw_Lies <- subset(
  aggregate(tmax ~ doy, subset(weather_Lies, year >= 2013), mean),
  doy %in% 61:152
)
...
DRD24_Lies <- as.numeric(optimize(fun24_Lies,
  interval = c(1, 365),
  maximum = FALSE
)[1])
t.DRD24_Lies <- w_Lies[w_Lies$doy.n == round(DRD24_Lies), ]$tmax
tpred24_Lies <- as.numeric(optimize(fun24_Lies,
  interval = c(1, 365),
  maximum = FALSE
)[2])
with(
  subset(w_Lies, doy.n <= 60),
  plot(tmax ~ doy.n,
    ylab = "Maximum Temperature (°Celsius)",
    xlab = "Day of the Year", pch = 19, cex = 0.8,
    xlim = c(-91, 150), bty = "L",
...
mw_Kyot <- subset(
  aggregate(tmax ~ doy, subset(weather_Kyot, year >= 2013), mean),
  doy %in% 61:152
)
...
DRD24_Kyot <- as.numeric(optimize(fun24_Kyot,
  interval = c(1, 365),
  maximum = FALSE
)[1])
t.DRD24_Kyot <- w_Kyot[w_Kyot$doy.n == round(DRD24_Kyot), ]$tmax
tpred24_Kyot <- as.numeric(optimize(fun24_Kyot,
  interval = c(1, 365),
  maximum = FALSE
)[2])
with(
  subset(w_Kyot, doy.n <= 60),
  plot(tmax ~ doy.n,
    ylab = "Maximum Temperature (°Celsius)",
    xlab = "Day of the Year", pch = 19, cex = 0.8,
    xlim = c(-91, 150), bty = "L",
...
mDRDW <- train(bloom_doy ~ DRD,
  data = dat_Wash,
  method = "lm", trControl = tc
)
# summary(mDRDW) # R^2 = 0.13

mtpredW <- train(bloom_doy ~ t.pred,
  data = dat_Wash,
  method = "lm", trControl = tc
)
# summary(mtpredW) # R^2 = 0.25

mjanW <- train(bloom_doy ~ mean.jan,
  data = dat_Wash,
  method = "lm", trControl = tc
)
# summary(mjanW) # R^2 = 0.02

mAllW <- train(bloom_doy ~ year + DRD + t.pred + mean.jan,
  data = dat_Wash,
  method = "lm", trControl = tc
)
...
data24_Wash <- data.frame(
  year = 2024, DRD = DRD24_Wash, t.pred = tpred24_Wash,
  mean.jan = j24_Wash
)
...
mYearL <- train(bloom_doy ~ year,
  data = dat_lies,
  method = "lm", trControl = tc
)
# summary(mYearL) # R^2 = 0.22
mDRDL <- train(bloom_doy ~ DRD,
  data = dat_lies,
  method = "lm", trControl = tc
)
# summary(mDRDL) # R^2 = 0.34
mtpredL <- train(bloom_doy ~ t.pred,
  data = dat_lies,
  method = "lm", trControl = tc
)
# summary(mtpredL) # R^2 = 0.57
mjanL <- train(bloom_doy ~ mean.jan,
  data = dat_lies,
  method = "lm", trControl = tc
)
# summary(mjanL) # R^2 = 0.08
mAllL <- train(bloom_doy ~ year + DRD + t.pred + mean.jan,
  data = dat_lies,
  method = "lm", trControl = tc
)
# summary(mAllL) # R^2 = 0.72
mAllL2 <- train(bloom_doy ~ year + DRD * t.pred + mean.jan,
  data = dat_lies,
  method = "lm", trControl = tc
)
...
plot(bloom_doy ~ t.pred, dat_lies,
  cex = 0.5, pch = 19,
...
data24_Lies <- data.frame(
  year = 2024, DRD = DRD24_Lies, t.pred = tpred24_Lies,
  mean.jan = j24_Lies
)
...
mYearK <- train(bloom_doy ~ year,
  data = dat_kyot,
  method = "lm", trControl = tc
)
# summary(mYearK) # R^2 = 0.24
mDRDK <- train(bloom_doy ~ DRD,
  data = dat_kyot,
  method = "lm", trControl = tc
)
# summary(mDRDK) # R^2 = 0.40
mtpredK <- train(bloom_doy ~ t.pred,
  data = dat_kyot,
  method = "lm", trControl = tc
)
# summary(mtpredK) # R^2 = 0.33
mjanK <- train(bloom_doy ~ mean.jan,
  data = dat_kyot,
  method = "lm", trControl = tc
)
# summary(mjanK) # R^2 = 0.05
mAllK <- train(bloom_doy ~ year + DRD + t.pred + mean.jan,
  data = dat_kyot,
  method = "lm", trControl = tc
)
# summary(mAllK) # R^2 = 0.76
mAllK2 <- train(bloom_doy ~ year + DRD * t.pred + mean.jan,
  data = dat_kyot,
  method = "lm", trControl = tc
)
...
pbloomK <- predict(mAllK2, newdata = data.frame(
  year = dat_kyot$year,
...
### Predict Bloom Date 2024
data24_Kyot <- data.frame(
  year = 2024, DRD = DRD24_Kyot, t.pred = tpred24_Kyot,
  mean.jan = j24_Kyot
)
...
m_wash <- train(
  First_Yes_DOY ~ year + Latitude + Longitude +
...
data_wash <- data.frame(
  year = unique(dat$year), Latitude = 38.8853,
...
doy <- data.frame(
  doy.pred = doy.pred, doy.actual = doy.actual,
...
plot(doy.pred ~ year, doy,
  ylim = c(80, 100), pch = 19, ylab =
...
arrows(
  x0 = doy$year, x1 = doy$year, y0 = doy$doy.pred, y1 = doy$doy.actual,
...
data_vanc <- data.frame(
  year = unique(dat$year), Latitude = 49.2237,
...
mw_Vanc <- subset(
  aggregate(tmax ~ doy, subset(weather_Vanc, year >= 2013), mean),
  doy %in% 61:152
)
...
DRD24_Vanc <- as.numeric(optimize(fun24_Vanc,
  interval = c(1, 365),
  maximum = FALSE
)[1])
t.DRD24_Vanc <- w_Vanc[w_Vanc$doy.n == round(DRD24_Vanc), ]$tmax
tpred24_Vanc <- as.numeric(optimize(fun24_Vanc,
  interval = c(1, 365),
  maximum = FALSE
)[2])
with(
  subset(w_Vanc, doy.n <= 60),
  plot(tmax ~ doy.n,
    ylab = "Maximum Temperature (°Celsius)",
    xlab = "Day of the Year", pch = 19, cex = 0.8,
    xlim = c(-91, 150), bty = "L",
...
### Different Models
mYearV <- train(bloom_doy ~ year,
  data = dat_Vanc,
  method = "lm", trControl = tc
)
# summary(mYearV) # R^2 = 0.61
mDRDV <- train(bloom_doy ~ DRD,
  data = dat_Vanc,
  method = "lm", trControl = tc
)
# summary(mDRDV) # R^2 = 0.01
mtpredV <- train(bloom_doy ~ t.pred,
  data = dat_Vanc,
  method = "lm", trControl = tc
)
# summary(mtpredV) # R^2 = 0.08
mjanV <- train(bloom_doy ~ mean.jan,
  data = dat_Vanc,
  method = "lm", trControl = tc
)
# summary(mjanV) # R^2 < .01
mAllV <- train(bloom_doy ~ year + DRD * t.pred + mean.jan,
  data = dat_Vanc,
  method = "lm", trControl = tc
)
...
### Predict Bloom Date 2024
data24_Vanc <- data.frame(
  year = 2024, DRD = DRD24_Vanc, t.pred = tpred24_Vanc,
  mean.jan = j24_Vanc
)
...
data_ny <- data.frame(
  year = unique(dat$year), Latitude = 40.73040,
...
mw_NY <- subset(
  aggregate(tmax ~ doy, subset(weather_NY, year >= 2013), mean),
  doy %in% 61:152
)
...
DRD24_NY <- as.numeric(optimize(fun24_NY,
  interval = c(1, 365),
  maximum = FALSE
)[1])
t.DRD24_NY <- w_NY[w_NY$doy.n == round(DRD24_NY), ]$tmax
tpred24_NY <- as.numeric(optimize(fun24_NY,
  interval = c(1, 365),
  maximum = FALSE
)[2])
with(
  subset(w_NY, doy.n <= 60),
  plot(tmax ~ doy.n,
    ylab = "Maximum Temperature (°Celsius)",
    xlab = "Day of the Year", pch = 19, cex = 0.8,
    xlim = c(-91, 150), bty = "L",
...
### Different Models
mYearN <- train(bloom_doy ~ year,
  data = dat_NY,
  method = "lm", trControl = tc
)
# summary(mYearV) # R^2 = 0.61
mDRDN <- train(bloom_doy ~ DRD,
  data = dat_NY,
  method = "lm", trControl = tc
)
# summary(mDRDV) # R^2 = 0.11
mtpredN <- train(bloom_doy ~ t.pred,
  data = dat_NY,
  method = "lm", trControl = tc
)
# summary(mtpredV) # R^2 = 0.13
mjanN <- train(bloom_doy ~ mean.jan,
  data = dat_NY,
  method = "lm", trControl = tc
)
# summary(mjanV) # R^2 = 0.27
mAllN <- train(bloom_doy ~ year + DRD * t.pred + mean.jan,
  data = dat_NY,
  method = "lm", trControl = tc
)
summary(mAllV) # R^2 = 0.63
...
### Predict Bloom Date 2024
data24_NY <- data.frame(
  year = 2024, DRD = DRD24_NY, t.pred = tpred24_NY,
  mean.jan = j24_NY
)
