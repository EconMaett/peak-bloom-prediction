...
Growing Degree Days are particularly useful for predicting cherry blossom peak bloom, especially for varieties like the Yoshino cherry trees (Prunus x yedoensis) that are famous in Washington DC's National Cherry Blossom Festival.
For cherry trees, the accumulation of GDD typically starts being counted from March 1st in the Northern Hemisphere. The base temperature used for cherry blossoms is usually 40°F (4.4°C). Peak bloom is generally predicted to occur when a specific threshold of accumulated GDD is reached.
For Yoshino cherry trees specifically:

Peak bloom typically occurs around 300 GDD (using base 40°F)
The "green bud" stage appears around 50 GDD
Florets become visible around 100 GDD
Extension of florets occurs around 150 GDD
Peduncle elongation happens around 250 GDD

Factors that can affect the accuracy of GDD predictions for cherry blossoms include:

Winter chill accumulation (trees need adequate cold exposure)
Extreme temperature fluctuations
Late freezes or unusual warm spells
Local microclimate conditions

The National Park Service uses this method, along with other factors, to make their annual peak bloom predictions for the DC cherry blossoms. However, they've found that predictions become most reliable when the trees reach the "green bud" stage.

Prunus x yedoensis (Yoshino Cherry): The most well-documented case, typically uses base `40°F` (`4.4°C`)
...
$$
\textrm{GDD} =  \frac{\mathrm{T}_{\mathrm{max}}+\mathrm{T}_{\mathrm{min}}}{2} - \mathrm{T}_{\mathrm{base}} 
$$
...
$$
\mathrm{AGDD}_{N} = \sum_{k=1}^{N} \mathrm{GDD}_{k}
$$
...
**NOTE**: Change the location names to "Kyoto", "Liestal", "New York City", "Vancouver", "Washington DC".
...
japan |>
  filter(location == "kyoto") |> 
  ggplot(mapping = aes(x = year, y = bloom_doy)) +
  geom_step(linetype = "dotted", colour = "#f72585", lwd = 0.75) +
...
From the map at <https://finder.vcbf.ca/> we get the following information:
- Blooming: `03/18-04/01`
- Cultivar: Akebono
- Location: Maple Grove Park
- Description: Kerrisdale - Akebonos at SE corner Maple Grove Park at Yew and SW Marine Dr

So these trees are currently predicted to bloom between `2025-03-18` and `2025-04-01`.

Searching the forum, we can confirm the official peak bloom dates for the past three years, which occurred on `2022-03-27`, `2023-04-07` and `2024-03-23`, respectively.

We can confirm that sprint 2017 was very cold in Vancouver from historical data sets.

Let's add the following dates to the data set:
...
# Add these observations to the cherry dataset
cherry_no_nyc <- cherry_no_nyc |> 
  bind_rows(vancouver_forum) |> 
  fill(lat, long, alt) |> 
  arrange(year)
```

Now we have eleven observations for Vancouver, BC.
...
cherry_with_nyc <- cherry_no_nyc |>
  bind_rows(nyc_data |> filter(year != 2024)) |> # Can't add same year twice!
  fill(lat, long, alt)
```
...
cherry_with_nyc <- cherry_no_nyc |>
  bind_rows(nyc_data) |>  # Can't add a year twice!
  fill(lat, long, alt)
```
...
**NOTE**: If you are satisfied with the augmented data set, save it:

```{r}
cherry <- cherry_with_nyc
```
...
The function `get_daily_avg_noaa()` will load from the dataset ID `GHCND` the main data type IDs `PRCP,SNOW,SNWD,TMAX,TMIN,ASTP,AWND`.
...
The Subject Matter Expert from the NASA POWER Team responded and said **there is no set rate limit**. However, the NASA POWER Team monitors the usage and will limit usage to support equitable access.

Thus, for the NOAA CDO data, it's best to only load the whole data set once and then add single days.

For NASA POWER, you can load the same data set repeatedly.
...
Read in the historical NOAA data:
...
Let's plot the daily maximum temperatures. Be aware that there are again implicit missing values that you need to make explicit `NA`s first!

Here I opt for the pivot-wider approach:

```{r}
historic_noaa_wide <- historic_noaa |> 
  select(-c("attributes", "station")) |> 
  pivot_wider(names_from = datatype, values_from = value) |> 
  mutate(TAXN = (TMAX + TMIN) / 2)

historic_noaa_wide |> 
  select(location, date, TMAX, TMIN, TAVG, TAXN) |> 
  mutate(TAVG = if_else(is.na(TAVG), TAXN, TAVG)) |> 
  mutate(TAXN = if_else(is.na(TAXN), TAVG, TAXN)) |> 
  filter(is.na(TMAX), location == "kyoto") |> arrange(desc(date))
```
...
historic_noaa |>
  ...
  facet_grid(rows = vars(location))
```
We have downloaded the following variables at a daily frequency:
...
Let's see if we can create Growing Degree Days (GDD).

We should first create `TAXN` = (`TMAX` + `TMIN`) / 2

**If the average temperature for a day is lower than the base temperature, then no Growing Degree Days are counted**.

Growing Degrees are accumulated daily, following a specified start date, by adding each day's total to all previous days' totals.

Chilling Degree Days (CDD) are used to quantify the accumulation of cold temperatures needed by certain plants (like fruit trees) to break dormancy after winter. The goal is to track hours or days with temperatures below a specific threshold, usually referred to as the chilling requirement for the plant.

For cherries, the chilling requirement is often associated with temperatures between `0°C` and `7°C` (`32°F` and `44.6°F`), although the exact range can vary based on the variety and environmental conditions.

For chilling degree days, we assume that the onset of dormancy is **October 1st** each year

The **2011 study** claims that **Yoshino** cherry trees with a base temperature of `4.3` degrees Celsius need about `78.9` chilling days since October 1st and `221.2` anti-chilling days, also called Growing Degree Days or Forcing days.

```{r}
base_temp <- 4.3 # 4.4 or 4.3 For Yoshino Cherries

historic_noaa_wide <- historic_noaa |> 
  select(-c("attributes", "station")) |> 
  pivot_wider(names_from = datatype, values_from = value) |> 
  mutate(TAXN = (TMAX + TMIN) / 2) |> 
  mutate(TAXN = if_else(is.na(TAXN), TAVG, TAXN)) |> # If TMAX or TMIN missing, use TAVG
  mutate(Tb = base_temp) |> 
  mutate(year = year(date)) |> 
  select(location, year, date, TAXN, Tb) |> 
  mutate(GDD = TAXN - Tb) |> 
  mutate(GDD = if_else(GDD < 0, 0, GDD)) |>
...
historic_noaa_wide |> 
  ...
  filter(location == "kyoto", year == 2002, AGDD >= 221.2) |> 
  slice_min(date)
```
...
```{r}
cherry |> 
  filter(location == "kyoto", year == 2002)
```
...
The meteorological data were obtained with R from the [Prediction of Worldwide Energy Resources (POWER)](https://power.larc.nasa.gov/#resources) project, using the [`nasapower`](https://docs.ropensci.org/nasapower/) for extraction of time series (since 1981-01-01 with daily frequency of the following variables:
...
```{r}
df_climate_newyork <- get_historic_power(
  var_climate = var_climate,
  start_date = start_date,
  end_date = end_date,
  frequency_climate = frequency_climate,
...
Combine the data for the five main locations:
